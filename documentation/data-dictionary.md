...
| **StopPlan** | trade / order, actual-stop price, **safety-stop** price (= max-DD-per-trade), promotion proximity, staged state (hidden / native), OCO linkage. **Implemented — plan + promotion decision** (gh#11 inc 4): entity `StopPlanRecord` (table `StopPlans`), **one per order** (unique FK, cascade), created when an entry transmits — the working stop starts **`Hidden`** while the safety stop already rests natively (inc 3's bracket). Carries side, entry, actual/safety stops, and the promotion band (**ticks** or a **fraction of the entry→stop distance** — *never* a % of raw price; **ATR is refused** until the indicator pipeline lands, R-3). The domain `StopPlan.ShouldPromote(price)` is the deterministic promotion decision, and `Promote()` is one-way + idempotent. **DB CHECKs** mirror the domain: **safety-beyond-actual** (side-dependent cross-column — the catastrophic floor must rest further from entry than the working stop, or it triggers first), staging ≠ unknown, metric ≠ unknown, band > 0. Skipped entirely when the two stops coincide — nothing to stage. **Deferred:** the live **promotion watcher** (needs quotes, R-1), OCO-cancel-on-exit, take-profit brackets, connection-liveness orphan handling | REL | ADR-0007, R-5 |
