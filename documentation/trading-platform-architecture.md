...
than drops, and releases the key itself when it has to (gh#1077) — but not in every case: a page whose delivery
straddles the refusal (in flight when it lands, or failed and re-offered later under a fresh ordinal) re-arms the
key with nothing left to release, so this re-arm is the belt over that residue and **stays**. It reports and never acts: no socket is torn down or disabled
