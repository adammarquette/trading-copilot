...
that is connected but never synced **no longer reads as a quiet account**: the stream refuses on
`TradovateTradingSocketSync.IsSynced` as well as on the connection state, and ends the stream when a reconnect
lands unsynced in the window between its checks and its handler attach — reporting that cause rather than the
drop's, because a message that names the wrong cause is a defect on a path whose justification is that the message
*is* the trace (gh#1051).
...
degraded, and the only trace is a repeating log line" is not trading-specific. Three consecutive passes that end
still owing raise one P2 through `INotificationChannel`, and the first pass that owes nothing **resolves** it —
without the resolve, the process-lifetime dedup would deliver the first outage and suppress every later one
(gh#1045, gh#1051). It reports and never acts: no socket is torn down or disabled on the host's own judgement. Each host supplies only its **post-connect obligation**, which differs in kind: a per-key replay that must
...
| `AccountEventStreamHost` | supervised venue stream | ingests fills / order-state events — writes `Fill` rows, advances orders to `PartiallyFilled`/`Filled`, drives OCO-cancel-on-exit (gh#219), and **journals the closed round trip to `Trade`** on a flat, with a signed tick-value-aware `RealizedPnL` (gh#731) — **deferring and retrying** that journal when a flat beats its closing fill into the stream, so a late fill does not lose the round trip (gh#748) | protection must retire off **venue truth**, not off what the app believes it sent; the same venue-truth fills are what the round trip is reconstructed from |
...
constructor, so absent credentials degrade that venue's feed instead of failing startup. That resolve, and the whole
poll loop around it, live once in `TradovateSocketConnectionHost` — the two hosts are subclasses, not copies, after
the copies diverged on the backoff reset within a week of shipping, gh#1054.) Each reads
