...
- **Primary wire:** `GET /api/1/equity-extended/market-news` — `headline`/`summary`/`source`/`url`/`datetime` (unix epoch)/`related` (comma tickers). Same shape as the Finnhub general-news adapter. Company-news is 1 ticker/request. Sentiment is aggregated scores (no URL) — not `INewsSource`. Press releases (`GET /api/1/research/press-releases/{ticker}`) are a News-category sibling with **no `url`** — same drop rule; do not invent a dedup key.
...
## [2026-07-22] ground | ProjectX/Topstep account-name families — stage resolution (gh#76)
**Source:** first-party observation — a real 294-account Topstep roster pulled read-only through `/connections/{id}/accounts/discover` (probe workspace, torn down after; names aggregated with digits masked, never dumped).
**Updated:** `pages/projectx-gateway-api.md` — replaced the "deliberately conservative, patterns deferred" note with the grounded **Account name families** table: `PRAC`/`PRACTICE<mon>#` → Practice, `<size>KTC`/`S<step><mon>#` → Evaluation, `EXPRESS…` → **Funded**; six families, **zero unmatched**.
**Key evidence:** every `EXPRESS` (funded) account is closed with a negative final balance; all 133 legacy `S#` evals are `CanTrade=false`; the only active accounts are 5 KTC evals + 1 PRAC — corroborating each family's reading. Direction-of-error argued on the page (misread-as-Funded = over-cautious; near-misses stay `Unknown`).
**Caveat recorded:** grounded against **Topstep only** — revisit when onboarding another ProjectX-family firm; the per-account override (gh#76) outranks the patterns and is the correction path.

## [2026-07-22] lint | Access-header backfill — terms posture recorded on every page (gh#119)
**Trigger:** licensing review against the public Apache-2.0 posture (README § How we work): `index.md` claimed the terms posture is "recorded in each page's `Access:` header" while **12/15 pages had none**, and the two 403-grounded pages cited gh#53 as open after it closed (completed, 2026-07-20).
**Updated:** `Access:` headers added to the 12 pages missing them, from how each source was actually obtained per this log (direct fetch / web-search-grounded / operator-provided / local-PDF-read). Verified for the two Microsoft sources: `dotnet/docs` and `microsoft/code-with-engineering-playbook` are both **CC-BY-4.0** (attributed summaries satisfy it; recorded in those pages' headers). `prop-firm-rules.md` + `take-profit-trader.md` now point the still-open Apex/TPT terms decision at **gh#118** (successor to gh#53); `index.md` notes the QuantConnect PDF was **purged from history** (gh#52), not merely removed.
**Verified clean in the same review:** no vendor document anywhere in git history (`rev-list --objects --all`); no substantial verbatim text on any page; quotes short + attributed inline.

## [2026-07-22] ground | 403-source terms reviewed — Apex & TPT postures decided (gh#118)
**Sources:** https://apextraderfunding.com/terms-of-use/ (interactive browser session — renders normally; the 403 blocks only non-browser fetches) ; https://takeprofittrader.com/terms (one direct fetch of the main site, `200` — the minimal read needed to learn its restriction).
**Decision (operator, 2026-07-22): keep both pages as-is** — the web-search-grounded summaries comply with both sets of terms; postures recorded in each page's `Access:` header, closing the gh#53 → gh#118 thread.
**Key findings:** **Apex ToU (eff. 2025-10-22)** — no anti-automation clause; public pages licensed for personal informational browsing; reproducing content needs written permission (none reproduced); a no-linking clause exists — citation URLs kept deliberately (facts, attribution). **TPT ToS** — prohibits automated access to its Resources and reproduction of content ⇒ never machine-fetch TPT properties; nothing was ever taken from them, and figures confirm in-portal (authenticated human access). **Neither firm restricts restating program rules.**
**Forward rule:** a 403 stays a refusal — don't route around it with fetch tools; Apex public pages may be read in an interactive browser (expressly licensed), TPT only in-portal by the operator.
