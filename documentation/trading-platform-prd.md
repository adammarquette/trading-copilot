...
**R-18: Multi-user authentication & authorization.** The Internet-exposed API — REST and real-time — requires **authentication** on every request/connection: a user **registers** and **logs in**, and the client carries a **JWT**; anonymous access is refused. The app is **multi-user** — every request is **authorized against the authenticated user** and data access is **scoped to that user** (R-20). Authorization runs through a **claims/policy layer** (RBAC-capable), so richer roles (teams, admins) can be added later without a major overhaul.
- [ ] Users **self-register** (email + credential) and **log in**; a session issues the JWT the client carries
