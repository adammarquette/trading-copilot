...
- **Billing / monetization & team roles.** The app is **multi-user** — each user registers a login and owns an isolated set of accounts, rules, and journal (R-18 / R-20) — but **paid plans, subscriptions, and billing are out of scope** for now, as are **team / admin role hierarchies** beyond per-user isolation (the claims/policy layer stays ready for them).
...
- [ ] **Sentiment scoring is deferred** (not on the free tiers): candidate enrichments are a **subagent classifier** (cheap model — ADR-0008) and/or a **user thumbs-up/down** rating that feeds the learning loop (R-9)
...
**R-18: Multi-user authentication & authorization.** The Internet-exposed API — REST and real-time — requires **authentication** on every request/connection: a user **signs in** and the client carries a **JWT** — onboarding is **invitation-only** (below); anonymous access is refused. The app is **multi-user** — every request is **authorized against the authenticated user** and data access is **scoped to that user** (R-20). Authorization runs through a **claims/policy layer** (RBAC-capable), so richer roles (teams, admins) can be added later without a major overhaul.
- [ ] **Onboarding is invitation-only:** the operator issues an **invitation** (bound to the invitee's email); the invitee **accepts it to create their account** (email + credential) and then **logs in** — there is **no open self-service sign-up**. A session issues the JWT the client carries
- [ ] An **invitation** is **single-use** and **expiring**; accepting it provisions the user's **isolated workspace** (R-20). An unused invitation can be **revoked**, blocking that sign-up
...
**R-20: Multi-user tenancy & data isolation.** Each user owns a **private, isolated** workspace — their own prop-firm **connections & accounts**, **trading rules**, **risk profiles**, **suggestions & orders**, **journal**, **chat**, and **AI-usage**; no user can see or affect another's. Isolation is **enforced server-side at the data layer** (every operator-owned row carries an owning user; every query is scoped to the authenticated user), **not just the UI**. **Reference & market data** (instruments, venues, data providers, bars/ticks/quotes, raw news) is **shared / global**. See [ADR-0011](adr/0011-multi-user-tenancy.md).
- [ ] Every **operator-owned** entity (connection, account, risk profile, rule/trigger, suggestion, order, trade & journal, conversation, relevance config, AI-usage, audit) is owned by exactly one **user** and is **inaccessible** to others
