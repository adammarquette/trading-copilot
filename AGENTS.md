...
- **Maximal metadata on every issue & PR/MR.** Opening one, populate every field it offers: **assign the current account** (the account the agent authenticates as); **set the milestone** when the phase is known (the milestones *are* the phases — Phase 0–4, Cross-cutting); **add it to the Project** and set its board fields (opening a PR moves its issue to *Review*); apply the `work:*` + `Work Estimate` labels; link the issue (`Closes #N`). A thin issue/PR is a defect — the next agent rebuilds context from these fields, so err toward more.
