# AGENTS.md — Platform Agent (CI/CD + infrastructure)

The **Platform Agent** contract, governing the pipeline and everything that runs the system. The root
`AGENTS.md` still applies. Like the [Code Reviewer](../AGENTS.md), this contract is **role-scoped rather than
subtree-scoped** — it sits here because the workflows are the densest artifact, but it owns them wherever they
live:
