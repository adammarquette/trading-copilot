...
public class TradeJournalServiceTests
{
    ...
    {
        // gh#809 (#823 review). A SINGLE-sided instant group can still be Guid-order-dependent: a long 3 reversed by a
        // sell filling 2,1,1 vs 1,1,2 at ONE instant passes running exposure through zero mid-group in one order but
        // not the other. The mixed-side guard does not fire (all sells), so slicing at that zero would journal only
        // the post-tie short and DROP the long cycle's P&L in one order while composing BOTH legs in the other -- the
        // same order-dependence gh#809 is about. Only slicing at a group's CLEAN END (order-independent net) fixes it:
        // the whole window composes deterministically into the long leg AND the short leg, identically either order.
        Guid owner = Guid.NewGuid();
        Guid accountId = await SeedAccountAsync(owner, "9001");
