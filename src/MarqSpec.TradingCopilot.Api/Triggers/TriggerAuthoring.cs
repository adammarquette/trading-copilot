...
/// <summary>
/// The <b>one</b> set of authoring refusals for a trigger's condition half (gh#1059, R-7) — the symbol, comparison,
/// period, resolution, indicator and hysteresis checks that <see cref="TriggerEndpoints.CreateTriggerAsync"/> has
/// always made, lifted out so a <i>second</i> author can make exactly the same ones.
/// </summary>
/// <remarks>
/// <para>
/// It exists because the chat <c>edit_rulebook</c> tool (gh#1059) authors triggers too, and a second copy of these
/// rules would drift: gh#1007 already had to fix the same threshold gap twice (create <b>and</b> patch), and a
/// model-authored trigger is exactly the caller you least want validated by the older copy.
/// </para>
/// <para>
/// Deliberately <b>one refusal per check</b> rather than one whole-request validator: each caller keeps its own
/// evaluation ORDER, so lifting these out changed no endpoint behaviour — the create endpoint still refuses a bad
/// route before a bad period, and the tool refuses in the order its own schema reads. The refusal strings are the
/// endpoint's verbatim, so a caller that surfaces them (the API's 400 body) reads exactly as before. Pure: no I/O,
/// no clock, no database — the account / route / size rules stay with their callers, since they differ.
/// </para>
/// </remarks>
internal static class TriggerAuthoring
{
    /// <summary>
    /// The R-22 indicator names a trigger may name (case-insensitive in, canonical out), so the stored name is
    /// exactly the <c>IIndicatorSource</c> read identity.
    /// </summary>
    ...
    /// <summary>Refuses a blank / unparseable instrument symbol.</summary>
