...
public static class TriggerEndpoints
{
    ...
    {
        // The condition half's refusals are TriggerAuthoring's (gh#1059) -- shared with the chat edit_rulebook tool
        // rather than copied, so a model-authored trigger can never be checked by an older copy of these rules
        // (gh#1007 is the precedent: the same threshold gap had to be fixed at create AND patch). The ORDER is this
        // endpoint's own and unchanged -- a bad route still refuses before a bad period -- because each refusal is
        // its own call rather than one whole-request validator.
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Comparison is { } comparison && TriggerAuthoring.RefuseComparison(comparison) is { } patchComparisonError)
        {
