...
/// <summary>
/// The <c>generate_suggestion</c> chat tool (gh#1059, R-4 / R-6, ADR-0029) — the co-pilot <b>proposes</b> a setup on
/// request, staged as an <c>Active</c> <see cref="Suggestion"/> the operator then reads, takes, or ignores.
/// </summary>
/// <remarks>
/// <para>
...
/// <see cref="SuggestionValidity"/>'s clamp against the market's auto-flatten deadline — the same three "the system's,
/// never the model's" values the scan's staging path sets. The model's prices are checked by the same
/// <see cref="SuggestionGeometry"/> the scan applies, and an incoherent proposal <b>fails closed</b>: nothing is
...
/// the shared options — deliberately <b>not</b> the chat endpoint's request context — so staging can neither flush the
/// endpoint's pending conversation writes early nor surface a CHECK violation on the endpoint's later
/// <c>SaveChanges</c> (a constraint backstops only its own transaction's owner). Owner-scoped by construction (R-20):
/// the context is built for the caller, so every read and the write are the operator's own.
/// </para>
/// </remarks>
public sealed class GenerateSuggestionTool : IChatTool
{
    ...
    public LlmToolDefinition Definition => new(
        ...
        + "choose the position size, the trading mode, or how long it stays valid — the system sets those. A "
        + "proposal whose stop or target is on the wrong side of entry is rejected and nothing is staged.",
        ...
        + "\"confidence\":{\"type\":\"integer\",\"description\":\"Your confidence, 0-100. Display only - it changes nothing about size or risk.\"},"
    ...
    public async Task<string> ExecuteAsync(string inputJson, CancellationToken cancellationToken)
    {
        ...
        // LAYER ONE, and it is the SAME check the scan makes (SuggestionGeometry): a self-evidently broken proposal --
        // a non-positive price, or a stop/target on the wrong side of entry -- is refused before anything is written.
        // Fail CLOSED: no partial row, and no "staged but incoherent" card for the operator to trip over.
        ...
        try
        {
            ...
            if (candidates.Count > 1)
            {
                ...
                // against, and guessing is exactly the kind of choice that must not become the model's by default.
                return Error(
            ...
            Suggestion staged = new()
            {
                ...
                // market's auto-flatten deadline.
            ...
            // Surfaced, not taken (ADR-0029): push the card to the owner AFTER the write commits. Presentation-only
            // and best-effort per ADR-0021 -- a hub fault must never unwind a durable proposal.
    ...
    /// refused as a tool error rather than surfacing as a write failure.
    /// </summary>
