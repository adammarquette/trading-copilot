...
- **A new suite gets a row in the inventory, in the same PR.** [`documentation/integration-test-audit.md`](../../documentation/integration-test-audit.md) §2 is the map the next agent reads to answer *"is this path already covered?"*, and `./scripts/check-integration-test-inventory.sh` **fails CI** when a suite has no row, a row names a file that is not there, or the stated count is stale (gh#862). Say what the suite guards and which failure mode it can fail on — a row that only restates the file name leaves the map no better than the omission did. Run it locally before the PR.
