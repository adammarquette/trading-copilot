...
- **A new suite gets a row in the inventory, in the same PR.** [`documentation/integration-test-audit.md`](../../documentation/integration-test-audit.md) §2 is the map the next agent reads to answer *"is this path already covered?"*, and `./scripts/check-integration-test-inventory.sh` **fails CI** when a suite is not the **subject** of a row (naming it inside another row's description does not count), a suite is the subject of **more than one** row (two rows can disagree, and a reader acts on the stale one — a group row listing several suites at once is exempt), a row's subject names a file that is not there, **two suite files share a basename** — the inventory keys rows on file name, so it cannot describe both, and no row you add will fix it — or the stated count is stale (gh#862, gh#948, gh#952). **A row is a claim about a suite that exists; the check cannot tell you a row's *status* is stale**, so when a suite ships, find the proposal row that asked for it and retire it. Say what the suite guards and which failure mode it can fail on — a row that only restates the file name leaves the map no better than the omission did. Run it locally before the PR.
- **A row's own "proven by name" identifiers stay honest, checked mechanically, not by re-reading.** `check-integration-test-inventory.sh` verifies a row's *subject* — that the suite exists — never a row's *content*: when a migration renames a `CK_`/`IX_`/`FK_`/`ct_` object a row names by hand, the row keeps the old name and nothing notices (gh#979, gh#762's rename). `Data/InventoryIdentifierLiveSchemaIntegrationTests.cs` closes that gap in the **integration (pre-merge) tier** — slower than the SDK-free inventory guard, because it needs a live migrated Postgres, but the only authority true by construction for every kind (`pg_constraint` / `pg_indexes` / `pg_trigger`; grepping the tree cannot fail since migrations are append-only, and the ModelSnapshot false-positives on EF's conventional FK/index names and on raw-SQL constraint triggers, neither of which it ever spells) (gh#981, gh#952's documented limit). It harvests §2's identifiers via a regex over the doc's own text, so **naming a new database object "by name" in §2 costs nothing extra** — the next PR renaming or dropping one is caught here, in the same tier, without a second list to keep in sync.
