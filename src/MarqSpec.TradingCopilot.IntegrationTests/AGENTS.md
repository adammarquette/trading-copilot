...
- **A new suite gets a row in the inventory, in the same PR.** [`documentation/integration-test-audit.md`](../../documentation/integration-test-audit.md) §2 is the map the next agent reads to answer *"is this path already covered?"*, and `./scripts/check-integration-test-inventory.sh` **fails CI** when a suite is not the **subject** of a row (naming it inside another row's description does not count), a row's subject names a file that is not there, **two suite files share a basename** — the inventory keys rows on file name, so it cannot describe both, and no row you add will fix it — or the stated count is stale (gh#862, gh#948). Say what the suite guards and which failure mode it can fail on — a row that only restates the file name leaves the map no better than the omission did. Run it locally before the PR.
