...
public interface IOrderExecutor : IVenue
{
    ...
    /// <remarks>
    /// <para>
    /// A <b>reducing</b> action, so it rides the same native close seam as <see cref="ClosePositionAsync"/> and
    /// <b>not</b> the order-placement path. An opposing order would be a <i>send</i>, gated by R-5 / R-16 / R-12 —
    /// and a gate refusing a risk-<i>lowering</i> action is a different thing from refusing to <i>raise</i> risk,
    /// and the wrong answer (gh#928). This transmits a close; it does not decide.
    /// </para>
    /// <para>
    ...
    /// nothing — either of which would be the opposite of what the caller asked for. An adapter that can size one,
    /// and advertises <see cref="VenueCapability.ReducePosition"/>, overrides this.
    /// </para>
    /// <para>
    /// Returns the venue's own post-reduce snapshot so the caller <b>verifies</b> the reduction against venue
    /// truth (ADR-0013) rather than inferring it from the call returning: a partial close the venue accepted while
    /// still reporting the original size is not a reduction, and must never read as done.
    /// </para>
    /// </remarks>
    /// <param name="account">The account holding the position.</param>
