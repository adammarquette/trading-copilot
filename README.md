...
A personal, single-operator **futures day-trading co-pilot** — a decision-support *and* execution system with a
...
> **Status: early / scaffolding.** The **`documentation/` folder is the current source of truth** — the product
> requirements and engineering practices live there. The .NET solution under `src/` is a stub today. This project
> deliberately reuses the architecture and engineering practices of its predecessor, **agent-forge-copilot**,
> built with an AI-Engineering-first approach.
...
| Doc | What's there |
|---|---|
| [`documentation/trading-platform-prd.md`](documentation/trading-platform-prd.md) | **Product requirements (PRD)** — problem, goals, the `R-1…R-17` requirements, success metrics, phasing, open questions |
| [`documentation/trading-platform-engineering.md`](documentation/trading-platform-engineering.md) | **Engineering guide** — the AgentForge architecture mapping, a lightweight engineering-practices scaffold (stack, testing, observability, deployment, safety-critical discipline), and the companion knowledge wiki |
| [`AGENTS.md`](AGENTS.md) | Instructions for AI coding agents (imported by `CLAUDE.md`) — settled conventions and where to find things |

The PRD is the source of truth for *what* the product does; the engineering guide is *how* we build it. Neither
is a spec the running system reads — they inform design.
...
## Stack (planned — inherited from agent-forge-copilot)
...
- **Postgres + pgvector** over EF Core — relational, vector, and time-series storage.
...
This project carries forward and sharpens the practices proven in **agent-forge-copilot** (its architectural
...
- **Work tracked in GitHub issues & PRs** — issue-first, no orphaned PRs (the practices agent-forge-copilot established on GitLab, carried over to GitHub here).

## Related projects

- **agent-forge-copilot** — the sibling .NET AI-copilot project this one inherits its architecture and
  engineering practices from; the reference for how the pieces fit together.
- **`MarqSpec.Client.ProjectX`** — the ProjectX / TopstepX client used for market data, account state, and order
  execution.

---

*Personal single-operator tooling — decision-support and execution for its operator, not investment advice for
third parties.*
